...
function App() {
  ...
  const hoverHandler = (e) => {
    if (e.target.dataset.label) {
      ...
      setTitle(key);
      if (`${key}` === "age") {
        setContent(person.dob.age);
      } else if (`${key}` === "street") {
        setContent(person.location.street.name);
      } else if (`${key}` === "password") {
        setContent(person.login.password);
      } else if (`${key}` === "name") {
        setContent(
          `${person.name.title} ${person.name.first} ${person.name.last}`
        );
      } else {
        setContent(person[`${key}`]);
      }
    }
  };
  ...
  const newUserHandler = () => {
    getData();
    setTitle("name");
    setContent(`${person.name.title} ${person.name.first} ${person.name.last}`);
  };
    ...
    <main>
      ...
      <div className="block">
        <div className="container">
          <img
            src={person.picture ? person.picture.medium : defaultImage}
            alt="random user"
            className="user-img"
          />
